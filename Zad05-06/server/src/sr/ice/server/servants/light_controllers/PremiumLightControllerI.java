...
public class PremiumLightControllerI extends LightControllerI implements IPremiumLightController {
    ...
    public PremiumLightControllerI() {
        super(new PremiumLightController(new DeviceMetadata("PremiumLightController",
                ThreadLocalRandom.current().nextInt(SERIAL_MAX)), new LightControllerData(LightMode.normal,
                new LightMode[]{LightMode.off, LightMode.normal, LightMode.musicSync, LightMode.videoSync}),
                new PremiumLightControllerData(new HashMap<>(Map.of(0, true, 1, true, 2, false, 3, false)))));
